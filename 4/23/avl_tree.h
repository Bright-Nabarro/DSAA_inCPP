...
public:
	template<typename U>
	friend void check_property(const AvlTree<U>& d);
	template<typename U>
	friend int check_avl(uptr_view<typename AvlTree<U>::AvlNode> node, int allowImbalance);
};
...
//23=========================================================================================
template<typename T>
void check_property(const AvlTree<T>& tree)
{
	check_avl<T>(tree.root, tree.sk_allowImbalance);
}

template<typename T>
int check_avl(uptr_view<typename AvlTree<T>::AvlNode> ptr, int allowImbalance)
{
	if(ptr == nullptr)
		return -1;

	int leftHeight = check_avl<T>(ptr->left, allowImbalance);
	int rightHeight = check_avl<T>(ptr->right, allowImbalance);
	
	int height = std::max(leftHeight, rightHeight) + 1;
	if(height != ptr->height)
		throw std::runtime_error{"height cannot catched"};
	if(std::abs(leftHeight - rightHeight) > allowImbalance)
