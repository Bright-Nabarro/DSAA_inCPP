...
public:
	...
	void draw(std::ostream& os)
	{
		...
		if(root == nullptr)
			return;
		draw(root, counterx, os);
	}
private:
	std::pair<int, int> draw(uptr& node, int& counter, std::ostream& os)
	{
		std::pair<int, int> cor, leftCor, rightCor;
		if(node->left != nullptr)
			leftCor = draw(node->left, counter, os);

		cor = {counter++, node->height+1};
		...
		if(node->left != nullptr)
			os << std::format("DrawLine({},{},{},{})\n", cor.first, cor.second, leftCor.first, leftCor.second);
		if(node->right != nullptr)
			os << std::format("DrawLine({},{},{},{})\n", cor.first, cor.second, rightCor.first, rightCor.second);
		return cor;
	}
