...
public:
    template<typename U>
    friend bool ordered(typename U::uptr& ptr);
...
template<typename T>
bool ordered(typename T::uptr& ptr)
{
    bool resultL{true}, resultR{true};
    if(ptr->left != nullptr)
        resultL = (ptr->left->element < ptr->element) && ordered<T>(ptr->left);
    if(ptr->right != nullptr)
        resultR = (ptr->element < ptr->right->element) && ordered<T>(ptr->right);
